...
def train_model(config):
    ...
    train_loader = DataLoader(rna_model.h5dataset(config['dataset'],'train')
                    ,num_workers=4,pin_memory=True,batch_size = config['batch_size'])
    valid_loader = DataLoader(rna_model.h5dataset(config['dataset'],'valid')
                    ,num_workers=4,pin_memory=True,batch_size = config['batch_size'])

    model = eval('rna_model.'+config['model'])(120,config['lr'])
