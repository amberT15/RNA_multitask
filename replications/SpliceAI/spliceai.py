...
def ResidualUnit(l, w, ar):
    ...
    def f(input_node):
        ...
        conv1 = Conv1D(l, int(w), dilation_rate=int(ar), padding='same')(act1)
        bn2 = BatchNormalization()(conv1)
        act2 = Activation('relu')(bn2)
        conv2 = Conv1D(l, int(w), dilation_rate=int(ar), padding='same')(act2)
...
def categorical_crossentropy_2d(y_true, y_pred):
    ...
    return - kb.mean(tf.cast(y_true[:, :, 0],tf.float32)*kb.log(y_pred[:, :, 0]+1e-10)
                    + tf.cast(y_true[:, :, 1],tf.float32)*kb.log(y_pred[:, :, 1]+1e-10)
                    + tf.cast(y_true[:, :, 2],tf.float32)*kb.log(y_pred[:, :, 2]+1e-10))
